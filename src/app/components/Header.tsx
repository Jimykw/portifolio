    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isMenuOpen
            ? 'border-b border-white/[0.06] bg-background shadow-[0_8px_32px_rgba(0,0,0,0.35)]'
            : scrolled
      ...
      {isMenuOpen && (
        ...
        >
          <nav className="flex flex-col h-full pt-20 px-4 pb-8 overflow-y-auto">
            <div className="space-y-1">
              {navItems.map((item) => (
                <button
                  ...
                  type="button"
                  onClick={() => scrollToSection(item.id)}
                  className={`block w-full text-left px-4 py-3.5 rounded-xl text-sm font-medium transition-colors ${
                    activeSection === item.id
